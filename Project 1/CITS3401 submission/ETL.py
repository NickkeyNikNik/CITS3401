...
def delete_files():
    ...
    files_to_delete = ['npu_DIM.csv', 'dates_DIM.csv', 'complete_data.csv',
                       'zone_DIM.csv', 'street_DIM.csv', 'crimeInfo_DIM.csv',
                       'crime_FACT.csv']
    """
...
def combine_files(folder_path, output_file):
    ...
    print(
        "Written to CSV file:", output_file)
...
def crimes_FACT():
    ...
    tmp_df = pd.merge(date_df, tmp_df, on=['date'], how='inner').drop(
        ['date', 'year', 'quarter', 'month'], axis=1)
